...
variables `weekday_weekend`, `week`, `day_id` and `day`; and there are
`activity_` indicates the activity minutes; and there are 1440 variables
...
The data contains 2595176 rows and 7 columns. The data has weather
information from 1981/1/1 to 2010/12/31, with variables such as prcp,
snow, snwd, tmax, and tmin.

The units for each variable are: `id` weather station id; `prcp`
precipitation (tenths of mm); `snwd` snow depth (mm); `tmax` maximum
temperature (tenths of Celsius degrees); `tmin` minimum temperature
(tenths of Celsius degrees);
...
<img src="p8105_hw3_jy3186_files/figure-gfm/unnamed-chunk-15-1.png" width="90%" />
